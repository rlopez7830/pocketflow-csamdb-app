...
2. **Flow Design**: Outline at a high level, describe how your AI system orchestrates nodes.
    ...
    - Outline the flow and draw it in a mermaid diagram. For example:https://github.com/intel-sandbox/refactor_pocketflow_template.githttps://github.com/intel-sandbox/refactor_pocketflow_template.git
      ```mermaid
      flowchart LR
...
def call_llm(prompt):
    ...
    return response
```
# Adding AGS Authentication to a FastAPI Web App
...
3. In the fast API web app, create a login route that first checks if a valid token is provided by the user. If so, it redirects the user to the main route '/'. Otherwise it redirects the user to the MSAL authorization url. Set a state cookie to prevent CSRF attacks.

4. Create a callback route that MSAL will redirect to after the user logs in. This route should handle the token exchange and set the token as an HTTP only cookie. It should also set an expiration time for the cookie based on the token's expiration time or if that is not found, set it to 1 hour. This should also be stored as a cookie. The callback route should only set the relevant cookies if the user has been successfully authenticated and it is ensured that the user's groups match at least one of the azure app assigned groups. If not, user should be directed to an error page saying authentication failed. Make sure to check the value of the state cookie to prevent CSRF attacks and delete this cookie after checking its value. 

5. Make every protected endpoint check for the presence of a token and validate it using the 'ags-auth' library. If the token is missing or invalid, the user should be redirected to the login route.

6. Delete the cloned repository for ags-auth after the implementation is complete. 

## Considerations
- DO NOT use the MSAL or any microsoft authentication libraries directly. Only use the 'ags-auth' library for all authentication related tasks. It has everything needed to authenticate a user and validate their token.
- The config.yaml file with the azure app details will be provided by the user in the workspace. Inside it, there will be a REDIRECT_URI provided. Please use that as the auth callback endpoint and change the port and host of the app accordingly. DO NOT change anything in the config.yaml file.
- There is no need for a separate login page. Only the login route is needed to handle authentication.
