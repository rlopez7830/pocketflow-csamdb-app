...
> 2. Include only the necessary utility functions, based on nodes and flows.
...
> Notes for AI:
> List the command line options supported by this tool
- `--help` or `-h`: Show help message and exit
- `--add <task>`: Add a new task
- `--delete <task_id>`: Delete a task by its ID
- `--update <task_id> <fields>`: Update a task's fields
- `--list`: List all tasks